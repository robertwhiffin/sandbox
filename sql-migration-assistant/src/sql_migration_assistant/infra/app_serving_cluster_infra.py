...
class AppServingClusterInfra:
    ...
    def choose_serving_cluster(self):
        question = (
            "The review application needs a cluster to run on. It is recommended to create a new, single node cluster "
        ...
        if choice.lower() == "y":
            ...
            clusters = {
                cluster.cluster_name: cluster.cluster_id for cluster in clusters
            }
            if clusters == {}:
    ...
    def _create_cluster(self):
        ...
        logging.info(
            f"See cluster state at {self.w.config.host}/compute/clusters/{response.response.cluster_id}"
        )
        return response
