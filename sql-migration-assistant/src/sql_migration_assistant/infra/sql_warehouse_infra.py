...
class SqlWarehouseInfra:
    ...
    def choose_compute(self):
        ...
        """
        warehouses = [
            f"CREATE A NEW SERVERLESS WAREHOUSE: {self.default_sql_warehouse_name}"
        ]
        ...
        warehouses.extend(
            [
                f"Name: {warehouse.name},\tType: {warehouse.warehouse_type.name},\tState: "
        ...
        question = (
            "Choose a warehouse: please enter the number of the warehouse you would like to use.\n"
        ...
        if "CREATE A NEW SERVERLESS WAREHOUSE" in choice:
            logging.info(
                f"Creating a new serverless warehouse {self.default_sql_warehouse_name}."
            )
            print(
                f"Creating a new serverless warehouse {self.default_sql_warehouse_name}."
            )
            ...
            warehouseID = _.id
            logging.info(
                f"Warehouse status visible at {self.w.config.host}/sql/warehouses/{warehouseID}"
            )
            print(
                f"Warehouse status visible at {self.w.config.host}/sql/warehouses/{warehouseID}"
            )
