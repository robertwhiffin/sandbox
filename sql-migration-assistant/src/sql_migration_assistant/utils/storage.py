...
@ensure_config
def read(table: str, config=None,columns: list[str] = None, where: str = None) -> pd.DataFrame:
    select_clause = "*" if columns is None else ','.join(columns)
    where_clause = "" if where is None else f" WHERE {where}"
    return pd.read_sql_query(f"SELECT {select_clause} FROM {config.schema}.{table}{where_clause}", config.con)

@ensure_config
def construct_upsert_query(table: str, values: list[dict[str, Any]], keys: list[str], columns: list[str], config) -> str:
    if keys is None:
...
@ensure_config
def construct_insert_query(table: str, values: list[dict[str, Any]], config=None) -> str:
    values_clause = ",".join([f"""('{"','".join([v for v in row.values()])}')""" for row in values])
...
@ensure_config
def insert(table: str, values: list[dict[str, Any]], config=None, keys: list[str]=None, upsert: bool=False):
    if upsert:
        # Get column order of base table
        columns = [c.name for c in config.w.tables.get(f"{config.schema}.{table}").columns]
